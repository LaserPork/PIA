...
<c:when test="${not empty requestScope.requests}">
	<c:forEach items="${requests}" var="friendReq">
	<div class="col-lg-3"> 
	<div class="post">
		<div class="post-header post-pic">
			<div class="zoomable-img post-pic">
				<figure>
					<a href="${pageContext.request.contextPath}/profile/${friendReq.id}">
					<c:choose>
					    <c:when test="${empty friendReq.profileImage}">
					        <img alt="Profile" src="${pageContext.request.contextPath}/img/profile/profile.png">
					    </c:when>    
					    <c:otherwise>
					       	<img alt="Profile" src="${pageContext.request.contextPath}/img/profile/${friendReq.profileImage}">
					    </c:otherwise>
					</c:choose>
				</a>
				</figure>
			</div>
				<span class="post-header-text-name clear-left">${friendReq.username}</span><br/>
				<form class="mb-0" method="POST">
						<input type="hidden" value="${friendReq.username}" name="request" />
						<input class="m-0 mt-1 btn post-button col-12" value="Accept friend request" type="submit" >
				</form>
			</div>
	...
	<c:otherwise>
		<div class="col-md-2"></div> 
			<div class="col-md-8 post"> 
			       <h1 class="text-center float-none"> You have currently no friend requests </h1>	
			</div>  
		<div class="col-md-2"></div> 
	</c:otherwise>
</c:choose>
